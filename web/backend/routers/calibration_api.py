"""
캘리브레이션 데이터를 브라우저에 내려주는 HTTP 엔드포인트.

왜 필요한가:
    지금까지 캘리브레이션 데이터는 서버 안에서만 읽혔다 - game_ws.py /
    rhythm_game_ws.py 가 WebSocket 안에서 직접 db 를 조회해 시선 판정까지
    끝냈기 때문이다. 계산을 브라우저(JS)로 옮기면 브라우저가 이 데이터를
    가지고 있어야 하므로, 조회용 엔드포인트가 따로 필요하다.

    기존 WebSocket 경로는 전혀 바뀌지 않는다 - 순수하게 추가되는 읽기 전용
    엔드포인트다.

보안:
    다른 보호된 엔드포인트와 동일하게 세션 쿠키로 사용자를 식별한다
    (get_current_user_id). 로그인하지 않았으면 401 이고, 남의 캘리브레이션을
    조회할 방법은 없다 - user_id 를 클라이언트가 지정할 수 없기 때문.
"""
