...
from sessions.calibration_session import CalibrationSession

router = APIRouter()


@router.websocket("/ws/calibration")
async def calibration_ws(websocket: WebSocket):
    ...
    session = CalibrationSession(int(init["width"]), int(init["height"]), user_id)
