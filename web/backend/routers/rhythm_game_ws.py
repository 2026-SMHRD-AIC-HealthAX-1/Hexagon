...
from sessions.rhythm_game_session import RhythmGameSession, CalibrationNotFoundError

router = APIRouter()


@router.websocket("/ws/rhythm-game")
async def rhythm_game_ws(websocket: WebSocket):
    ...
    try:
        session = RhythmGameSession(user_id)
